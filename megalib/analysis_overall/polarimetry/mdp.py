...
if __name__ == '__main__':
    ...
    for source_energy in Energy_lst:
        result_polarimetry = f"{base_polarimetry_folder}/{source_type}{source_energy}keV_config{HED_config}x{HED_config}_{distance_dets}cm"
    ...
    for delta_t in time_lst:
        ...
        for mcrab, linestyle, color, marker in zip(mcrab_lst, linestyles, colors, markers):
            ...
            plt.errorbar(x, y, xerr=half_bin, capsize = 3, fmt=marker, color=color, label=format_flux_label(mcrab))
